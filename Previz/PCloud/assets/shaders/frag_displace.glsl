...
void main( void )
{
	...
	vec2 fUV = vec2(1.0-uv.s,1.0-uv.t);
	vec2 above = texture2D( buffer, fUV + vec2( 0.0, -pixel.y ) ).rg;
	float x = above.g - texture2D( buffer, fUV + vec2( pixel.x, 0.0 ) ).g;
	float y = above.r - texture2D( buffer, fUV + vec2( 0.0, pixel.y ) ).r;
